...
"""
    ...
    def __str__(self) -> str:
            """
            FileContextの文字列表現を返す
            
            LLMプロンプトに使用できる形式でプロジェクト情報を返します。
            
            Returns:
                str: プロジェクトサマリーの文字列表現
            """
            summary = self.get_project_summary()
            ...
            parts.append(f"Root: {summary['root_directory']}")
            parts.append(f"Total files: {summary['total_files']}")
            parts.append(f"Total lines: {summary['total_lines']}")
            ...
            if summary['languages']:
                ...
                parts.append(f"Languages: {languages_str}")
    ...
    def __repr__(self) -> str:
        ...
        Returns:
            str: デバッグ情報
        """
        return f"FileContext(root={self.root_dir}, files={len(self.files)})"
