...
class Memory:
    ...
    def get_conversation_context(self, limit: int = 5) -> List[Dict[str, str]]:
        ...
        for entry in reversed(recent_entries):
            ...
            # entry.claude_replyがNoneまたは空の場合はスキップ
            if entry.claude_reply and entry.claude_reply.strip():
                context.append({
                    "role": "assistant", 
    ...
    def save_memory(self):
        ...
        try:
            ...
            err_console.print(f"Warning: Failed to save memory: {e}")
    ...
    def load_memory(self):
        ...
        try:
            ...
            err_console.print(f"Warning: Failed to load memory: {e}")
            self.entries = []
    ...
    def save_repository_data(self, data: Dict[str, Any]) -> bool:
        """Persist repository data"""
        try:
            repo_file = self.memory_dir / "repository_data.json"
            with open(repo_file, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False, default=str)
            return True
        except Exception as e:
            err_console.print(f"âš ï¸ Failed to save repository data: {e}")
            return False

    def load_repository_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            err_console.print(f"âš ï¸ Failed to load repository data: {e}")
        return None

    def save_repository_index(self, index_data: Dict[str, Any]) -> bool:
        """Save repository index"""
        try:
            index_file = self.memory_dir / "repository_index.json"
            with open(index_file, 'w', encoding='utf-8') as f:
                json.dump(index_data, f, indent=2, ensure_ascii=False)
            return True
        except Exception as e:
            err_console.print(f"âš ï¸ Failed to save index: {e}")
            return False

    def load_repository_index(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            err_console.print(f"âš ï¸ Failed to load index: {e}")
        return None
    ...
    def get_repository_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analysis_version": stats.get("analysis_version", "1.0")
        }

    def clear_repository_data(self) -> bool:
        ...
        try:
            ...
            err_console.print("âœ… Repository data cleared successfully")
            return True
        except Exception as e:
            err_console.print(f"âš ï¸ Failed to clear repository data: {e}")
            return False

    def get_repository_data_size(self) -> Dict[str, Any]:
        """Get file size information for repository data"""
        try:
            repo_file = self.memory_dir / "repository_data.json"
            index_file = self.memory_dir / "repository_index.json"
            ...
            if repo_file.exists():
                sizes["repository_data"] = repo_file.stat().st_size
            if index_file.exists():
                sizes["repository_index"] = index_file.stat().st_size
                
            sizes["total"] = sizes["repository_data"] + sizes["repository_index"]
            
            return sizes
        except Exception as e:
            err_console.print(f"âš ï¸ Failed to get file sizes: {e}")
            return {"repository_data": 0, "repository_index": 0, "total": 0}