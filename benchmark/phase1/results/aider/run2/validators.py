...
def validate_no_circular_dependency(task_id: int, depends_on_id: int, 
    ...
    """Check for circular dependencies. Returns (valid, error_message)."""
    ...
    if has_path(depends_on_id, task_id, set()):
        return False, f"Circular dependency detected: task #{depends_on_id} already depends on task #{task_id}"
