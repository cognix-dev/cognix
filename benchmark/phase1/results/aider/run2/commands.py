    ...
    if "status" in updates:
        ...
        if updates["status"] == "done":
            ...
            if deps:
                ...
                if incomplete_deps:
                    print(f"WARNING: Task #{task_id} depends on incomplete tasks: {incomplete_deps}")
...
def cmd_list_recurring(**kwargs):
    ...
    for r in rules:
        ...
        print(f"Rule #{r.id}: {task_title} ({r.frequency}, {status}, next: {r.next_run[:10]}, created: {r.created_count}{end_info})")
...
def cmd_cancel_recurring(rule_id: int, **kwargs):
    """Cancel (deactivate) a recurring rule."""
    rules = load_recurring_rules()
    rule = None
    for r in rules:
        if r.id == rule_id:
...
def cmd_list_dependencies(task_id: int, **kwargs):
    ...
    if dependents:
        ...
        for d in dependents:
            ...
            if dep_task:
                print(f"    #{dep_task.id}: {dep_task.title}")
    else:
        print("\n  Blocks: (none)")
...
def cmd_dashboard(days: int = 7, user_id: Optional[int] = None, **kwargs):
    ...
    # Completion rate
    total_in_period = len(created_in_period)
    completed_count = len(completed_in_period)
    completion_rate = (completed_count / total_in_period * 100) if total_in_period > 0 else 0.0
    
    # Average completion time
    avg_completion_time = sum(completion_times) / len(completion_times) if completion_times else 0.0
    avg_hours = avg_completion_time / 3600
    
    # Most active user (by created + completed)
    user_activity = {}
