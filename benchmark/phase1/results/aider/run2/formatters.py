...
def format_dashboard(data: dict) -> str:
    """Format dashboard analytics data."""
    lines = [
        f"=== Activity Dashboard (Last {data['days']} days) ===",
        "",
        f"Tasks Created:       {data['created']}",
        f"Tasks Completed:     {data['completed']}",
        f"Completion Rate:     {data['completion_rate']:.1f}%",
        f"Avg Completion Time: {data['avg_completion_hours']:.1f} hours",
        "",
    ]
    ...
    if data['most_active_user']:
        ...
        lines.append("Most Active User:    (none)")
    
    if data['busiest_project']:
        lines.append(f"Busiest Project:     #{data['busiest_project']}")
    else:
        lines.append("Busiest Project:     (none)")
    
    lines.append("")
    lines.append(f"Overdue Tasks:       {data['overdue']}")
    lines.append("")
