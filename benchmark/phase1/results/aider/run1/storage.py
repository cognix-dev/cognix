...
def save_dependencies(deps: List[TaskDependency]):
    _save_json(DEPENDENCIES_FILE, [d.to_dict() for d in deps])


def get_dependencies_for_task(task_id: int) -> List[TaskDependency]:
    """Get all dependencies where task_id is blocked by other tasks."""
    return [d for d in load_dependencies() if d.task_id == task_id]


def get_dependents_of_task(task_id: int) -> List[TaskDependency]:
    """Get all dependencies where other tasks depend on task_id."""
    return [d for d in load_dependencies() if d.depends_on_id == task_id]


def get_next_dependency_id() -> int:
    deps = load_dependencies()
    return max((d.id for d in deps), default=0) + 1
