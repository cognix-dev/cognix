...
class TestRecurringTasks(BaseTestCase):
    ...
    def test_recurring_rule_advance_weekly(self):
        ...
        rule.advance()
        next_run_dt = datetime.fromisoformat(rule.next_run)
        self.assertGreater((next_run_dt - now).days, 6)
    ...
    def test_recurring_rule_advance_monthly(self):
        ...
        rule.advance()
        next_run_dt = datetime.fromisoformat(rule.next_run)
        self.assertGreater((next_run_dt - now).days, 25)
    ...
    def test_recurring_with_past_end_date_no_create(self):
        ...
        rule = models.RecurringRule(id=1, task_template_id=1, frequency="daily", 
                                    next_run=yesterday, end_date=last_week)
        storage.save_recurring_rules([rule])
        
        commands.cmd_run_recurring(user_id=1)
        
        tasks = storage.load_tasks()
        self.assertEqual(len(tasks), 1)  # No new task created
...
class TestTaskDependencies(BaseTestCase):
    ...
    def test_self_dependency_rejection(self):
        task = models.Task(id=1, title="Test", project_id=1)
        storage.save_tasks([task])
    ...
    def test_duplicate_dependency_rejection(self):
        task1 = models.Task(id=1, title="A", project_id=1)
        task2 = models.Task(id=2, title="B", project_id=1)
    ...
    def test_circular_dependency_direct(self):
        ...
        commands.cmd_add_dependency(1, 2)  # 1 depends on 2
        result = commands.cmd_add_dependency(2, 1)  # 2 depends on 1 (circular)
        self.assertFalse(result)

    def test_circular_dependency_transitive(self):
        task1 = models.Task(id=1, title="A", project_id=1)
        task2 = models.Task(id=2, title="B", project_id=1)
        task3 = models.Task(id=3, title="C", project_id=1)
        storage.save_tasks([task1, task2, task3])
        
        commands.cmd_add_dependency(1, 2)  # 1 depends on 2
        commands.cmd_add_dependency(2, 3)  # 2 depends on 3
        result = commands.cmd_add_dependency(3, 1)  # 3 depends on 1 (circular)
        self.assertFalse(result)
    ...
    def test_delete_task_removes_dependencies(self):
        ...
        commands.cmd_delete_task(1)
        deps = storage.load_dependencies()
        self.assertEqual(len(deps), 0)

    def test_status_change_warns_incomplete_dependencies(self):
        task1 = models.Task(id=1, title="A", project_id=1, status="todo")
        task2 = models.Task(id=2, title="B", project_id=1, status="todo")
        storage.save_tasks([task1, task2])
        
        commands.cmd_add_dependency(2, 1)  # 2 depends on 1
...
class TestActivityDashboard(BaseTestCase):
    ...
    def test_dashboard_counts_created_completed(self):
        ...
        task1 = models.Task(id=1, title="A", project_id=1, created_at=yesterday.isoformat())
        task2 = models.Task(id=2, title="B", project_id=1, status="done", 
    ...
    def test_dashboard_avg_completion_time(self):
        ...
        task = models.Task(id=1, title="A", project_id=1, status="done",
                          created_at=yesterday.isoformat(),
                          completed_at=now.isoformat())
        storage.save_tasks([task])
    ...
    def test_format_dashboard_sections(self):
        data = {
            "days": 7,
            "user_id": None,
            "tasks_created": 5,
