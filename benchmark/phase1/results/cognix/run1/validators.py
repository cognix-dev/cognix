...
from models import TaskStatus, TaskPriority, TaskDependency
...
def validate_frequency(frequency: str) -> Tuple[bool, str]:
    """Validate recurring frequency value"""
    valid = ["daily", "weekly", "biweekly", "monthly"]
    if frequency not in valid:
        return False, f"Frequency must be one of: {', '.join(valid)}"
    return True, ""
...
def validate_no_circular_dependency(
    ...
    # Check from the task_id node
    if has_cycle(task_id):
        return False, f"Circular dependency detected: adding this would create a cycle"
