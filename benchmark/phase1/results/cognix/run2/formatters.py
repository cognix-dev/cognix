...
def format_task_table(tasks: List[Task]) -> str:
    ...
    lines.append(f"{'ID':<5} {'Status':<12} {'Priority':<8} {'Title':<30} {'Assignee':<10} {'Due':<12}")
    lines.append(separator)
...
def format_dashboard(data: dict) -> str:
    ...
    lines = [
        ...
        "=" * 60,
        "",
        f"Period: Last {data.get('days', 7)} days",
        "",
        "TASK METRICS:",
        ...
        f"  Avg Completion Time: {data.get('avg_completion_time', 'N/A')}",
        "",
        "ACTIVITY LEADERS:",
        f"  Most Active User:    {data.get('most_active_user', 'N/A')}",
        f"  Busiest Project:     {data.get('busiest_project', 'N/A')}",
        "",
        "CURRENT STATUS:",
        f"  Overdue Tasks:       {data.get('overdue_count', 0)}",
        "",
        "TASKS BY STATUS:",
    ]
    ...
    for status, count in data.get('by_status', {}).items():
        icon = STATUS_ICONS.get(status, "?")
        lines.append(f"  {icon} {status:<15} {count}")
