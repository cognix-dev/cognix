...
def build_parser() -> argparse.ArgumentParser:
    ...
    # export
    p = sub.add_parser("export", help="Export tasks to file")
    ...
    # summary
    p = sub.add_parser("summary", help="Show task summary statistics")
    p.add_argument("--project", type=int, default=None)

    # recurring-add
    p = sub.add_parser("recurring-add", help="Create a recurring task rule")
    p.add_argument("task_id", type=int)
    p.add_argument("frequency", choices=["daily", "weekly", "biweekly", "monthly"])
    ...
    # recurring-run
    p = sub.add_parser("recurring-run", help="Process recurring rules and create tasks")
    p.add_argument("--user", type=int, required=True)
    ...
    p.add_argument("task_id", type=int, help="Task that will be blocked")
    p.add_argument("depends_on_id", type=int, help="Task that must be completed first")

    # dep-list
    p = sub.add_parser("dep-list", help="List dependencies for a task")
    p.add_argument("task_id", type=int)

    # dep-remove
    p = sub.add_parser("dep-remove", help="Remove a task dependency")
    p.add_argument("task_id", type=int)
    p.add_argument("depends_on_id", type=int)
...
def main():
    ...
    if args.command == "init":
        ...
        cmd_add_recurring(args.task_id, args.frequency, args.end_date)
        ...
        cmd_add_dependency(args.task_id, args.depends_on_id)
    elif args.command == "dep-list":
        cmd_list_dependencies(args.task_id)
    elif args.command == "dep-remove":
        cmd_remove_dependency(args.task_id, args.depends_on_id)
