...
class RecurringRule:
    ...
    def should_run(self) -> bool:
        ...
        # Check if next_run is today or in the past
    ...
    def advance(self) -> None:
        ...
        if self.frequency == "daily":
            ...
            if month > 12:
                month = 1
                year += 1
            # Keep same day, or last day of month if day doesn't exist
            day = min(current.day, self._last_day_of_month(year, month))
            next_date = current.replace(year=year, month=month, day=day)
        else:
            raise ValueError(f"Unknown frequency: {self.frequency}")

        self.next_run = next_date.date().isoformat()
