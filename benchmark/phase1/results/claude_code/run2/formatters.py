...
def format_dashboard(data: dict) -> str:
    """Format the dashboard data as a readable text report."""
    lines = [
        "=" * 60,
        f"Activity Dashboard (Last {data['days']} days)",
        "=" * 60,
        "",
        "Task Activity:",
    ...
    if data['average_completion_time'] is not None:
        ...
        lines.append(f"  Avg completion:    {' '.join(time_str)}")
    else:
        lines.append(f"  Avg completion:    N/A")

    lines.append("")

    if data['most_active_user']:
        lines.append(f"Most Active User: #{data['most_active_user']}")
    else:
        lines.append(f"Most Active User: N/A")

    if data['busiest_project']:
        lines.append(f"Busiest Project:  #{data['busiest_project']}")
    else:
        lines.append(f"Busiest Project:  N/A")

    lines.append("")
    lines.append(f"Overdue Tasks: {data['overdue_count']}")
