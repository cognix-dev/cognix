...
class RecurringRule:
    ...
    def should_run(self) -> bool:
        """Check if this rule should create a task now."""
        if not self.active:
            return False

        try:
            next_run_date = datetime.fromisoformat(self.next_run)
            now = datetime.now()
    ...
    def advance(self):
        """Update next_run based on frequency and increment created_count."""
        try:
            next_run_date = datetime.fromisoformat(self.next_run)
            ...
            if self.frequency == "daily":
                ...
                next_run_date += timedelta(weeks=2)
            elif self.frequency == "monthly":
                # Add roughly a month (30 days)
                next_run_date += timedelta(days=30)

            self.next_run = next_run_date.isoformat()
