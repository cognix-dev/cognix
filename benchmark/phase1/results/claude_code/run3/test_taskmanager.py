...
class TestRecurringTasks(BaseTestCase):
    ...
    def test_recurring_rule_should_run_active(self):
        """Test RecurringRule.should_run() returns True when conditions are met."""
        yesterday = (datetime.now() - timedelta(days=1)).isoformat()
    ...
    def test_recurring_rule_should_run_future(self):
        """Test RecurringRule.should_run() returns False for future next_run."""
        tomorrow = (datetime.now() + timedelta(days=1)).isoformat()
        rule = models.RecurringRule(
            id=1, task_template_id=1, frequency="daily",
    ...
    def test_recurring_rule_should_run_past_end_date(self):
        """Test RecurringRule.should_run() returns False when end_date is past."""
        yesterday = (datetime.now() - timedelta(days=1)).isoformat()
    ...
    def test_recurring_rule_advance_biweekly(self):
        ...
        rule.advance()
        expected = (now + timedelta(weeks=2)).isoformat()
        self.assertEqual(rule.next_run, expected)
        self.assertEqual(rule.created_count, 1)
    ...
    def test_cmd_run_recurring_creates_tasks(self):
        ...
        # Create a user, project, and template task
        user = models.User(id=1, username="alice", email="a@t.com")
        storage.save_users([user])
        project = models.Project(id=1, name="Test", owner_id=1)
        storage.save_projects([project])
        task = models.Task(id=1, title="Template", project_id=1, tags=["test"])
        storage.save_tasks([task])

        # Create a recurring rule that should run now
        yesterday = (datetime.now() - timedelta(days=1)).isoformat()
        rule = models.RecurringRule(
            id=1, task_template_id=1, frequency="daily",
            next_run=yesterday, active=True
        ...
        self.assertEqual(len(tasks), 2)  # Original + new task

        # Check that the rule was advanced
        rules = storage.load_recurring_rules()
        self.assertEqual(rules[0].created_count, 1)

    def test_recurring_with_past_end_date_no_tasks(self):
        """Test recurring with end_date in past does not create tasks."""
        ...
        # Create rule with past end_date
        yesterday = (datetime.now() - timedelta(days=1)).isoformat()
        two_days_ago = (datetime.now() - timedelta(days=2)).isoformat()
        rule = models.RecurringRule(
            id=1, task_template_id=1, frequency="daily",
...
class TestTaskDependencies(BaseTestCase):
    ...
    def test_add_dependency_rejects_self_dependency(self):
    ...
    def test_add_dependency_rejects_duplicate(self):
        """Test duplicate dependency rejection."""
        user = models.User(id=1, username="alice", email="a@t.com")
        storage.save_users([user])
        project = models.Project(id=1, name="Test", owner_id=1)
        storage.save_projects([project])
        task1 = models.Task(id=1, title="Task 1", project_id=1)
        task2 = models.Task(id=2, title="Task 2", project_id=1)
    ...
    def test_circular_dependency_detection_direct(self):
        """Test direct circular dependency detection (A→B→A)."""
        dep1 = models.TaskDependency(id=1, task_id=2, depends_on_id=1)
        existing = [dep1]

        ok, msg = validators.validate_no_circular_dependency(1, 2, existing)
        self.assertFalse(ok)
        self.assertIn("circular", msg.lower())

    def test_circular_dependency_detection_transitive(self):
        """Test transitive circular dependency detection (A→B→C→A)."""
        dep1 = models.TaskDependency(id=1, task_id=2, depends_on_id=1)
        dep2 = models.TaskDependency(id=2, task_id=3, depends_on_id=2)
        existing = [dep1, dep2]

        ok, msg = validators.validate_no_circular_dependency(1, 3, existing)
        self.assertFalse(ok)
    ...
    def test_cmd_delete_task_removes_dependencies(self):
        ...
        task1 = models.Task(id=1, title="Task 1", project_id=1)
        task2 = models.Task(id=2, title="Task 2", project_id=1)
        task3 = models.Task(id=3, title="Task 3", project_id=1)
        storage.save_tasks([task1, task2, task3])

        dep1 = models.TaskDependency(id=1, task_id=2, depends_on_id=1)
        dep2 = models.TaskDependency(id=2, task_id=3, depends_on_id=2)
        storage.save_dependencies([dep1, dep2])

        # Delete task 2
        commands.cmd_delete_task(task_id=2, user_id=1)

        # Check that dependencies involving task 2 are removed
    ...
    def test_status_change_warns_incomplete_dependencies(self):
        ...
        project = models.Project(id=1, name="Test", owner_id=1)
        storage.save_projects([project])
        task1 = models.Task(id=1, title="Task 1", project_id=1, status="todo")
        task2 = models.Task(id=2, title="Task 2", project_id=1, status="todo")
        storage.save_tasks([task1, task2])
        ...
        # Try to mark task 2 as done while task 1 is not done
        ...
        self.assertIn("depends on", output.lower())
...
class TestDashboard(BaseTestCase):
    ...
    def test_dashboard_counts_created_completed(self):
        ...
        now = datetime.now()
        task1 = models.Task(id=1, title="Task 1", project_id=1, created_at=now.isoformat())
        task2 = models.Task(id=2, title="Task 2", project_id=1,
                             created_at=(now - timedelta(days=2)).isoformat(),
                             status="done", completed_at=now.isoformat())
        # Task outside range (old)
        task3 = models.Task(id=3, title="Task 3", project_id=1,
                             created_at=(now - timedelta(days=10)).isoformat())

        storage.save_tasks([task1, task2, task3])

        # Dashboard should count task1 and task2 as created in range
        # and task2 as completed in range
        result = commands.cmd_dashboard(days=7)
        self.assertTrue(result)
    ...
    def test_dashboard_average_completion_time(self):
        ...
        task = models.Task(id=1, title="Task 1", project_id=1,
    ...
    def test_format_dashboard(self):
        ...
        data = {
            ...
            "avg_completion_time": timedelta(days=1, hours=5),
            "most_active_user": 1,
            "busiest_project": 1,
            "overdue_count": 2,
            "status_breakdown": {"todo": 2, "done": 3},
        }
