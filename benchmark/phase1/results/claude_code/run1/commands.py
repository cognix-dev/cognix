    ...
    if "status" in updates:
        ...
        if updates["status"] == "done":
            ...
            if deps:
                ...
                if incomplete_deps:
                    print(f"WARNING: Task #{task_id} has incomplete dependencies: {incomplete_deps}", file=sys.stderr)
...
def cmd_list_recurring(**kwargs):
    ...
    for r in rules:
        ...
        print(f"Rule #{r.id}: {status}, template: #{r.task_template_id} ({task_title}), "
              f"frequency: {r.frequency}, next: {r.next_run[:10]}, created: {r.created_count}{end_info}")
...
def cmd_list_dependencies(task_id: int, **kwargs):
    ...
    if deps:
        ...
        for d in deps:
            ...
            if dep_task:
                print(f"    #{dep_task.id}: {dep_task.title} (status: {dep_task.status})")
    else:
    ...
    if dependents:
        ...
        for d in dependents:
            dependent_task = get_task_by_id(d.task_id)
            if dependent_task:
                print(f"    #{dependent_task.id}: {dependent_task.title}")
    else:
        print("\n  No dependents (no tasks are waiting on this)")
...
def cmd_remove_dependency(task_id: int, depends_on_id: int, **kwargs):
    ...
    deps = [d for d in deps if not (d.task_id == task_id and d.depends_on_id == depends_on_id)]
    save_dependencies(deps)
...
# ====================================================================
# Activity Dashboard Commands
# ====================================================================

def cmd_dashboard(days: int = 7, user_id: Optional[int] = None, **kwargs):
    """Show activity dashboard with analytics."""
    tasks = load_tasks()
    now = datetime.now()
    start_date = now - timedelta(days=days)

    # Filter tasks by date range and user if specified
    created_in_period = []
    completed_in_period = []
    ...
    completion_rate = (tasks_completed / tasks_created * 100) if tasks_created > 0 else 0.0
    ...
    # Most active user
    user_activity = {}
    for t in created_in_period + completed_in_period:
        if t.assignee_id:
