...
# ====================================================================
# Feature 1: Recurring Tasks Tests
# ====================================================================
...
class TestRecurringTasks(BaseTestCase):
    ...
    def test_recurring_rule_should_run_future(self):
        """Test RecurringRule.should_run() returns False when next_run is in future."""
        tomorrow = (datetime.now() + timedelta(days=1)).isoformat()
        rule = models.RecurringRule(
            id=1, task_template_id=1, frequency="daily",
    ...
    def test_recurring_rule_advance_daily(self):
        ...
        initial_count = rule.created_count
        rule.advance()
        self.assertEqual(rule.created_count, initial_count + 1)
        next_run = datetime.fromisoformat(rule.next_run)
        self.assertGreater(next_run, start)
    ...
    def test_recurring_rule_advance_weekly(self):
        ...
        self.assertGreaterEqual(delta.days, 7)
    ...
    def test_recurring_rule_advance_biweekly(self):
        ...
        next_run = datetime.fromisoformat(rule.next_run)
        delta = next_run - start
        self.assertGreaterEqual(delta.days, 14)
    ...
    def test_recurring_with_end_date_in_past(self):
        """Test recurring rule with end_date in the past does not create tasks."""
        # Create a user, project, and template task
        user = models.User(id=1, username="test", email="test@test.com")
        storage.save_users([user])
        project = models.Project(id=1, name="Test", owner_id=1)
        ...
        # Create a recurring rule with end_date in the past
        yesterday = (datetime.now() - timedelta(days=1)).isoformat()
        two_days_ago = (datetime.now() - timedelta(days=2)).isoformat()
        rule = models.RecurringRule(
            id=1, task_template_id=1, frequency="daily",
        ...
        # Check no new tasks were created
        tasks = storage.load_tasks()
...
# ====================================================================
# Feature 2: Task Dependencies Tests
# ====================================================================
...
class TestTaskDependencies(BaseTestCase):
    ...
    def test_circular_dependency_direct(self):
        ...
        # Add A depends on B
        commands.cmd_add_dependency(task_id=1, depends_on_id=2)
    ...
    def test_circular_dependency_transitive(self):
        ...
        # Add A→B→C
        commands.cmd_add_dependency(task_id=1, depends_on_id=2)
        commands.cmd_add_dependency(task_id=2, depends_on_id=3)

        # Try to add C→A (circular)
        result = commands.cmd_add_dependency(task_id=3, depends_on_id=1)
        self.assertFalse(result)
    ...
    def test_delete_task_removes_dependencies(self):
        ...
        # Add dependencies
        commands.cmd_add_dependency(task_id=2, depends_on_id=1)
        commands.cmd_add_dependency(task_id=3, depends_on_id=1)
    ...
    def test_status_change_warns_incomplete_dependencies(self):
        ...
        # Try to mark task2 as done while task1 is not done
        f = io.StringIO()
...
# ====================================================================
# Feature 3: Activity Dashboard Tests
# ====================================================================
...
class TestActivityDashboard(BaseTestCase):
    ...
    def test_dashboard_counts_created_completed(self):
        ...
        # Create tasks in date range
        now = datetime.now()
        task1 = models.Task(id=1, title="Task1", project_id=1, assignee_id=1,
                           created_at=now.isoformat(), status="done",
                           completed_at=now.isoformat())
        task2 = models.Task(id=2, title="Task2", project_id=1, assignee_id=1,
    ...
    def test_dashboard_completion_rate(self):
        ...
        self.assertIn("50", output)  # 1/2 = 50%
    ...
    def test_dashboard_avg_completion_time(self):
        ...
        task1 = models.Task(id=1, title="Task1", project_id=1, assignee_id=1,
                           created_at=created.isoformat(), status="done",
                           completed_at=now.isoformat())
    ...
    def test_format_dashboard(self):
        ...
        self.assertIn("12.5 hours", output)
